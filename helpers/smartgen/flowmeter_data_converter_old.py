...
def process_dfm_data(register,address,serial_number):
    ...
    print(previous_log_status)
    if status or previous_log_status:
        payload = get_dfm_logs_payload(register,address,serial_number)
        #insert value in the dfm database
        sqlite_service.dfm_logs_insert(payload)
        #save the current values as the previous values
        sqlite_service.update_dfm_logs_current_value(payload)
            
    
def get_dfm_logs_payload(register,address,serial_number):
    ct=dt.datetime.now()
